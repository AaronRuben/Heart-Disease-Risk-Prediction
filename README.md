...
- add  more details about framingham heart study, from OG intro  - but maybe put this in conclusion / discussion? (depends on if we know what cohort this is from..)

Since 1947, the Framingham Heart Study (FHS) has been a central pioneer of the expansion from treatment-based to preventive medicine. By establishing risk factor determination as central pillar data analysis of studying disease, the FHS has led scientists all over the world to use data analysis to further our understanding of countless diseases, better enabling our doctors to issue the preventative care necessary to save lives[cite paper below]. As our technological era now provides the medical field with more data than ever before, it is necessary that machine learning is harnessed to help turn this information into knowledge through the creation of risk scores for disease. The methods of PCA, K-means clustering, and convolutional neural networks will assist us to not only better understand the FHS data, but also to create tools that could be used to predict cardiovascular disease into the future, which still remains as a primary cause of death around the globe. An important limitation of this project is that the FHS study primarily only focused on the white population of the city, until the Omni cohorts were added in 1995 [cite],  which hinders the study’s ability to generalize for a diverse population. However, by understanding how ML can best be used for this population, further projects can apply these ML methods to broader population groups, to study the risk factors associated with cardiovascular disease and any other diseases. 


### Data Description
 - add description of data, with the graphs aaron made describing it 
